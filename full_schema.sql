-- Complete Database Schema for AI Student Support Platform
...
-- Student Profiles
-- email + phone_number are required by app/app.py profile UPDATE.
...
-- Skills
-- Direct per-student skills used by app/app.py profile + load_student_recommendation_data().
-- Legacy catalog rows (student_id IS NULL) remain valid for student_skills mapping.
...
-- Opportunities (canonical cache of normalized external listings)
...
-- Fetched Opportunities (API cache)
...
-- Student Profile Updates (tracking profile changes)
...
-- Safe to run multiple times; fresh installs are already correct via CREATE above.
-- ---------------------------------------------------------------------------
DELIMITER $$
...
CALL sp_add_column_if_missing('student_profiles', 'email', '`email` VARCHAR(255) NULL');
CALL sp_add_column_if_missing('student_profiles', 'phone_number', '`phone_number` VARCHAR(50) NULL');
CALL sp_add_column_if_missing('skills', 'student_id', '`student_id` INT NULL');
CALL sp_add_column_if_missing('skills', 'skill_type', '`skill_type` VARCHAR(20) NULL');
